...
export default function DietDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 min-h-screen pb-24">
      {/* Header & Tabs */}
      <div className="flex flex-col md:flex-row justify-between items-center mb-10 bg-card/60 backdrop-blur-2xl border border-border p-6 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-primary/10 rounded-full blur-[100px] pointer-events-none" />
        
        <div>
          <h1 className="text-4xl font-black text-foreground tracking-tight mb-2 flex items-center gap-3">
            <Utensils className="w-8 h-8 text-primary" /> Nutrition Center
          </h1>
          <p className="text-muted-foreground font-medium">Your personalized hub for diet analytics and meal planning.</p>
        </div>
        
        <div className="mt-6 md:mt-0 flex p-1 bg-background/50 backdrop-blur-xl border border-border rounded-full shadow-inner">
          <button
            onClick={() => setActiveTab('analytics')}
            className={`px-8 py-3 rounded-full font-bold transition-all duration-300 ${activeTab === 'analytics' ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/30' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Analytics
          </button>
          <button
            onClick={() => setActiveTab('planner')}
            className={`px-8 py-3 rounded-full font-bold transition-all duration-300 ${activeTab === 'planner' ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/30' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Meal Planner
          </button>
        </div>
      ...
      {activeTab === 'analytics' && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* ROW 1: Trend & Macros */}
            <div className="lg:col-span-2 bg-card/50 backdrop-blur-2xl p-8 rounded-[2rem] shadow-2xl border border-border">
              <h3 className="text-xl font-black text-foreground mb-6 flex items-center gap-3 uppercase tracking-widest">
            ...
            <div className="bg-card/50 backdrop-blur-2xl p-8 rounded-[2rem] shadow-2xl border border-border">
              <h3 className="text-xl font-black text-foreground mb-6 flex items-center gap-3 uppercase tracking-widest">
              ...
              <div className="h-64 flex justify-center relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none flex-col mt-4">
                  <span className="text-3xl font-black text-foreground">{analytics.foodQuality.healthy}</span>
            ...
            <div className="lg:col-span-3 bg-gradient-to-br from-primary/10 via-secondary/10 to-accent/10 border border-border p-10 rounded-[3rem] shadow-2xl backdrop-blur-2xl relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-10 relative z-10">
                <h3 className="text-3xl font-black text-foreground tracking-tight">Today's Target Overview</h3>
                <button
                  onClick={saveToHistory}
                  className="bg-card text-foreground border border-border px-6 py-3 rounded-full font-bold hover:bg-muted transition-colors shadow-lg flex items-center gap-2"
                >
                  Save Log
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
                <div className="bg-card/80 backdrop-blur-md border border-border p-8 rounded-3xl hover:bg-card transition-colors shadow-xl">
                  <p className="text-primary font-black mb-3 uppercase tracking-widest text-sm flex items-center gap-2">Protein</p>
                  <p className="text-6xl font-black text-foreground tracking-tighter">{diet.macros.protein.grams}<span className="text-2xl text-muted-foreground ml-1">g</span></p>
                </div>
                <div className="bg-card/80 backdrop-blur-md border border-border p-8 rounded-3xl hover:bg-card transition-colors shadow-xl">
                  <p className="text-foreground font-black mb-3 uppercase tracking-widest text-sm flex items-center gap-2">Carbs</p>
                  <p className="text-6xl font-black text-foreground tracking-tighter">{diet.macros.carbs.grams}<span className="text-2xl text-muted-foreground ml-1">g</span></p>
                </div>
                <div className="bg-card/80 backdrop-blur-md border border-border p-8 rounded-3xl hover:bg-card transition-colors shadow-xl">
                  <p className="text-secondary font-black mb-3 uppercase tracking-widest text-sm flex items-center gap-2"><Droplet className="w-4 h-4"/> Fats</p>
                  <p className="text-6xl font-black text-foreground tracking-tighter">{diet.macros.fats.grams}<span className="text-2xl text-muted-foreground ml-1">g</span></p>
                </div>
              </div>
            </div>
          </div>
