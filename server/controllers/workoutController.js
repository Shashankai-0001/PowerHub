...
exports.createOrUpdateProfile = async (req, res) => {
    try {
        const { age, gender, fitnessGoal, experienceLevel, equipment, dailyDuration } = req.body;
        ...
        if (profile) {
            profile = await UserWorkoutProfile.findOneAndUpdate(
                { userId },
                { age, gender, fitnessGoal, experienceLevel, equipment, dailyDuration },
                { new: true }
            );
        } else {
            profile = new UserWorkoutProfile({
                userId, age, gender, fitnessGoal, experienceLevel, equipment, dailyDuration
            });
            await profile.save();
        }
        res.json(profile);
